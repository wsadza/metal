...
luisa::unique_ptr<Environment::Instance> Spherical::build(
    ...
    if (!_emission->is_constant()) {
        ...
        auto [alias_buffer_view, alias_buffer_id] = pipeline.bindless_buffer<AliasEntry>(aliases.size());
        auto [pdf_buffer_view, pdf_buffer_id] = pipeline.bindless_buffer<float>(pdfs.size());
